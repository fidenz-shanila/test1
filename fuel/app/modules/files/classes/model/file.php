   ...
   public static function get_cb_file_content($CF_FileNumber_pk)
    {
     ...
     /**
     * update CB File location list
     * @param
     * @author shanila
     */
       public static function update_cb_location_list($update_set,$CF_FileNumber_pk){
           if(strlen($update_set)!=0){
            $sql='UPDATE t_CbFile SET '.$update_set.' WHERE CF_FileNumber_pk =\''.$CF_FileNumber_pk.'\'';
           // print_r($sql);exit;
            \NMI::DB()->query($sql);
           }
            echo "<script>parent.$('body').css('overflow','auto'); parent.refrashListing(); parent.$('#FileLocation').dialog('close');</script>";exit;
		//print_r($sql);exit;
    }
