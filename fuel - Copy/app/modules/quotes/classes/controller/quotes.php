...
//print_r(json_encode ($data['owner']));exit;
   ...
   /**
    ...
    function action_new_quote($year_seq_pk_id=null)
    {
        ...
        $fs->add('offices',  'Offices  :', array('type' => 'select', 'class' => 'select-2 wdb_data add_reset_filter', 'options' => $offices ),  array(array('required')));
        ...
        $fs->add('types',    'Types    :', array('type' => 'select', 'class' => 'select-2 wdb_data ', 'id' => 'sel_type', 'options' => array(''=>'')),    array(array('required')));
        ...
        $fs->add('file_no', 'file_no :', array('type' => 'text', 'class' => 'textbox-1 cb_file_id','name'=>'txt_file_name', 'id'=>'txt_file_name', 'readonly' => 'readonly'), array(array('required')));
        $fs->add('title', 'Title :', array('type' => 'text', 'class' => 'textbox-1 cb_file_title', 'name' => 'txt_file_title', 'id'=>'txt_file_title', 'readonly' => 'readonly'), array(array('required')));
            ...
            { 	
              ...
              //  $stmt = $this->db->prepare("EXEC sp_insert_into_t_ArtefactAndQuote @WDB_B_Name=?, @WDB_S_Name=?, @WDB_P_Name=?, @WDB_A_Name=?, @WDB_TestOfficerEmployeeID=?, @A_Type=?, @A_Make=?,@A_Model=?,@A_SerialNumber=?,@A_Description=?,@A_CF_FileNumber_fk=?,@A_OR1_OrgID_fk=?,@A_ContactID=?,@Q_ServicesOffered=?,@Q_CertificateOffered=?,@Q_SpecialRequirements=?,@A_PerformanceRange=?,@A_YearSeq_pk=?");
                ...
                $stmt = $this->db->prepare(" SET NOCOUNT ON
                 ...
                 $stmt->bindParam('A_YearSeq_pk', $year_seq_pk_id, \PDO::PARAM_STR|\PDO::PARAM_INPUT_OUTPUT, 400);
...
//		exit;
          ...
          //print_r(\Input::get('sections'));exit;
            $data=array('section'=>\Input::get('sections'),'project'=>\Input::get('projects'),'area'=>\Input::get('sel_area'),'type'=>\Input::get('sel_type'));
            $listing = new Model_Quote;
