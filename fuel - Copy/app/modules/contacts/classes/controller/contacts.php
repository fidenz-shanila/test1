...
class Controller_Contacts extends \Controller_Base {
    ...
    function action_listing_data() 
    {
        ...
        {
                $data[] = array(
                    (\Input::get('mode'))?'<div  style="background-color:#8da7eb;width:100%;height:30px;padding:2px"><input type="button" class="select_item " data-org_id="' . $c['OR1_OrgID_pk'] . '" data-contact_id="' . $c['CO_ContactID_pk'] . '" data-org_name="' . $c['CO_Fullname'] .'" data-contact_name="' . $c['OR1_FullName'] . '" value="<<" /></div>':'<div  style="background-color:#8da7eb;width:100%;height:30px;padding:2px"><input type="button"  class="select_item spaced1" onclick=cleanSetIntervalFunction() id="' . $c['CO_ContactID_pk'] . '" data-id="' . $c['CO_ContactID_pk'] . '" value=".." /></div>',
...
echo '<pre>',print_r($e),'</pre>';exit;
    ...
    function action_new_org()
    {
    	...
    	$fs->add('names', 'Name :', array('type' => 'select', 'class' => 'select-1 editable', 'options' => $names ), array(array('required')));
    	...
    	$fs->add('createdby', 'Created By :', array('type' => 'text', 'class' => 'select-1', 'readonly' => 'readonly', 'value' => \NMI::current_user('full_name') ));
    	$fs->add('submit', 'Submit :', array('type' => 'submit', 'value' => 'insert', 'class' => 'button2','id'=>'IdSubmit'));
...
echo "<script> parent.$('#insertCheckr').val('$CO_ContactID_pk');parent.$('#dialog').dialog('close'); </script>";exit;
   ...
   echo '<input type="text" id="cherDelectValue" value="DELECTED"><script>parent.$(\'body\').css(\'overflow\',\'auto\'); window.parent.closeIframe();</script>';exit;
