...
class Controller_Files extends \Controller_Base {
    ...
    function action_listing_data() 
    {
        ...
        foreach ($result['result'] as $c) 
        {
            ...
            if (\Input::get('is_selectable')) {
                $action = \Form::input('select', '<<', array('type' => 'button', 'class' => 'select_file spaced', 'data-id' => $c['CF_FileNumber_pk'], 'data-title' =>  $c['CF_Title']));
            } else {
            ...
            $data[] = array(
                //$action,
                '<div style="width:100%; background-color:#ffff80;padding:2px; height:30px;"><input class="spaced1"  type="button" value=".." /></div>',
    ...
    function action_new_file()
    {
        ...
        $fs->add('type', 'Type :', array('type' => 'select', 'class' => 'select-1', 'options' => array('CB' => 'CB', 'MC' => 'MC', 'CG' => 'CG', 'id' => 'type'), array(array('required'))));
        $fs->add('optsite', '', array('options' => array('Syd', 'Melb'), 'type' => 'radio', 'class' => 'optsite', 'value' => 'true'), array(array('required')));
        ...
        $fs->add('fill_number', 'FILL NUMBER :', array('type' => 'text', 'class' => 'textbox-1', 'id' => 'fill' ), array(array('required'))); 
    ...
    function action_get_next()
    {
        ...
        $optsite = \Input::param('optsite');
        
        $data = Model_File::get_next_number($type, $optsite);
        $CF_FileNumber_pk = Model_File::build_number($type, $data['Next_Seq'], $data['Next_Year']);
    ...
    function action_file_location() 
    {
        $locations =  Model_File::get_file_location();
        
        $fs = \Fieldset::forge('');
        $fs->add('current_location', 'Current_Location :', array('type' => 'select', 'class' => 'select-1', 'options' => $locations, array(array('required'))));
        $fs->add('requested_location', 'Requested_Location :', array('type' => 'select', 'class' => 'select-1', 'options' => $locations, array(array('required'))));
